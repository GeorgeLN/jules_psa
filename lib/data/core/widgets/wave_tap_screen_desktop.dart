...
class WaveTapScreenDesktop extends StatefulWidget {
  const WaveTapScreenDesktop({super.key});

  @override
  State<WaveTapScreenDesktop> createState() => _WaveTapScreenDesktopState();
}

class _WaveTapScreenDesktopState extends State<WaveTapScreenDesktop> {
  ...
  Future<void> _loadImage() async {
    final ByteData data = await rootBundle.load('assets/images/silueta_humana_desktop.png');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          GestureDetector(
            onTapDown: (details) => _onTapDown(details, context, _imageKey),
            child: Stack(
              children: [
                Positioned.fill(
                  child: Image.asset(
                    'assets/images/silueta_humana_desktop.png',
                    key: _imageKey,
                    fit: BoxFit.contain,
                  ),
                ),
            ...
            child: Column(
              ...
              children: [
                ...
                IconButton(
                  ...
                  icon: Icon(Icons.delete, color: Colors.white, size: width * 0.035),
                ),
                IconButton(
                  onPressed: _removeLastWave,
                  icon: Icon(Icons.restart_alt_outlined, color: Colors.white, size: width * 0.035),
                ),
              ],
            ),
          ),
